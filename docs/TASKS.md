...
- [ ] W2 — validated data-driven definitions and exact Yarn-to-catalog event/choice parity tests are complete for all 100 staged Childhood/School/Career/Health/Money nodes. The launch plus staged event catalog now shares one runtime age-eligibility predicate, exercises every event at exact and adjacent age boundaries, and audits every follow-up target, probability, delay, cancellation rule, and reachable age window. Finish non-event age boundaries, seeded distribution, editorial review, and gradual measured registration before random selection.
