...
- [ ] W2 — validated data-driven definitions and exact Yarn-to-catalog event/choice parity tests are complete for all 100 staged Childhood/School/Career/Health/Money nodes. The launch plus staged event catalog shares one runtime age-eligibility predicate, exercises every event at exact and adjacent age boundaries, audits every follow-up contract, and measures deterministic weighted selection across childhood, school, teen, adult, and senior cohorts. Finish non-event age boundaries, editorial review, and gradual measured registration before random selection.
