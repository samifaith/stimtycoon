...
{
    public sealed class StimEventSheetBinderTests
    {
        [Test]
        public void Constructor_RequiresCompleteNamedContract()
        {
            Assert.Throws<ArgumentNullException>(() => new StimEventSheetBinder(null));
            Assert.That(new StimEventSheetBinder(new VisualElement()).IsValid, Is.False);
            Assert.That(new StimEventSheetBinder(CreateRoot()).IsValid, Is.True);
        }
        ...
        {
            var root = CreateRoot();
            var binder = new StimEventSheetBinder(root)
            {
