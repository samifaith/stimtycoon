...
{
    public static class StimEffectValueRules
    {
    ...
    public interface IStimEffectValueResolver
    {
        float Resolve(Effect effect);
        bool ContainsRule(string ruleId);
    }

    public sealed class StimEffectValueRuleDefinition
    {
        public string id;
    ...
    /// </summary>
    public sealed class StimEffectValueResolver : IStimEffectValueResolver
    {
        public const string BalanceProfileId = "staged_rewards_v1";
        private static readonly IReadOnlyDictionary<string, StimEffectValueRuleDefinition> Definitions =
            new Dictionary<string, StimEffectValueRuleDefinition>(StringComparer.Ordinal)
            {
                { StimEffectValueRules.StagedStatGain, Rule(StimEffectValueRules.StagedStatGain, 1f, 0f, 10f) },
                { StimEffectValueRules.StagedStatLoss, Rule(StimEffectValueRules.StagedStatLoss, -1f, -10f, 0f) },
                { StimEffectValueRules.StagedSkillXpGain, Rule(StimEffectValueRules.StagedSkillXpGain, 1f, 0f, 100f) },
                { StimEffectValueRules.StagedCareerProgressGain, Rule(StimEffectValueRules.StagedCareerProgressGain, 1f, 0f, 100f) },
                { StimEffectValueRules.StagedCashSmallGain, Rule(StimEffectValueRules.StagedCashSmallGain, 500f, 0f, 100000f) },
                { StimEffectValueRules.StagedCashMediumGain, Rule(StimEffectValueRules.StagedCashMediumGain, 1000f, 0f, 250000f) },
                { StimEffectValueRules.StagedCashSmallCost, Rule(StimEffectValueRules.StagedCashSmallCost, -500f, -100000f, 0f) }
            };

        private readonly Dictionary<string, float> values;

        public StimEffectValueResolver(IReadOnlyDictionary<string, float> overrides = null)
        {
            ...
            foreach (var item in overrides)
            {
                ...
                if (definition.requiresWholeUnits && Math.Abs(item.Value - (float)Math.Round(item.Value)) > float.Epsilon)
                    throw new ArgumentException($"Effect value rule {item.Key} requires whole units.", nameof(overrides));
        ...
        public static IReadOnlyList<StimEffectValueRuleDefinition> GetDefinitions() =>
            Definitions.Values.OrderBy(item => item.id, StringComparer.Ordinal).ToList();

        private static StimEffectValueRuleDefinition Rule(
            string id, float defaultValue, float minimumValue, float maximumValue) =>
            new StimEffectValueRuleDefinition
            {
                id = id,
