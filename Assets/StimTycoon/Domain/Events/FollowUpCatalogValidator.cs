...
{
    public sealed class FollowUpCatalogValidationResult
    {
    ...
    public static class FollowUpCatalogValidator
    {
        private static readonly Regex CancellationRule = new Regex(
            "^[a-z][a-z0-9_]*$", RegexOptions.Compiled | RegexOptions.CultureInvariant);

        public static FollowUpCatalogValidationResult Validate(IReadOnlyList<Event> events)
        {
            var result = new FollowUpCatalogValidationResult();
            if (events == null) return Fail(result, "Authored event catalog is null.");
            var byId = new Dictionary<string, Event>(StringComparer.Ordinal);
        ...
        private static bool CanAgeWindowsIntersect(
            Event source, Event target, ScheduledEventRef followUp)
        {
        ...
        private static string Range(Event evt) => evt.ageRange == null
            ? "missing"
            : $"{evt.ageRange.minAge}–{evt.ageRange.maxAge}";

        private static FollowUpCatalogValidationResult Fail(
            FollowUpCatalogValidationResult result, string error)
        {
            AddError(result, error);
            return result;
        }

        private static void AddError(FollowUpCatalogValidationResult result, string error)
        {
