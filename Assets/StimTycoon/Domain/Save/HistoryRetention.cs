...
namespace StimTycoon.Saves
{
    public static class HistoryRetention
    {
        ...
        public static void Apply(GameState state)
        {
            ...
            state.eventHistory ??= new List<EventHistoryEntry>();
            state.historyArchive ??= new HistoryArchiveState();
        ...
        private static int FindOldestRoutineEntry(IReadOnlyList<LifeFeedEntry> entries)
        {
        ...
        private static void AddMajorSummary(HistoryArchiveState archive, string summary)
        {
