...
namespace StimTycoon.Tests.Domain.Content
{
    ...
    public sealed class YarnAuthoringContractTests
    {
        ...
        {
            var catalog = RepresentativeEvents.CreateLaunchAlphaCatalog()
                .Concat(StagedEventCatalog.CreateChildhoodBatch())
                .Concat(StagedEventCatalog.CreateSchoolBatch())
                .Concat(StagedEventCatalog.CreateCareerBatch())
                .Concat(StagedEventCatalog.CreateHealthBatch())
                .Concat(StagedEventCatalog.CreateMoneyBatch())
        ...
        {
            var events = StagedEventCatalog.CreateAllStagedEvents();
        ...
        {
            foreach (var evt in StagedEventCatalog.CreateAllStagedEvents())
            {
        ...
        public void ChildhoodStagedBatch_HasValidatedCatalogAndExactChoiceParity()
        {
            var nodes = ReadNodes()
                ...
                .ToList();
            var events = StagedEventCatalog.CreateChildhoodBatch()
        ...
        public void ChildhoodStagedBatch_RemainsOutsideRandomLaunchCatalog()
        {
            var launchIds = new HashSet<string>(
                RepresentativeEvents.CreateLaunchAlphaCatalog().Select(evt => evt.id),
                StringComparer.Ordinal);

            Assert.That(StagedEventCatalog.CreateChildhoodBatch()
        ...
        public void SchoolStagedBatch_HasValidatedCatalogAndExactChoiceParity()
        {
            var nodes = ReadNodes()
                ...
                .OrderBy(node => node.eventId, StringComparer.Ordinal)
                .ToList();
            var events = StagedEventCatalog.CreateSchoolBatch()
                .OrderBy(evt => evt.id, StringComparer.Ordinal)
                .ToList();
        ...
        public void SchoolStagedBatch_RemainsOutsideRandomLaunchCatalog()
        {
            ...
            Assert.That(StagedEventCatalog.CreateSchoolBatch()
                .Any(evt => launchIds.Contains(evt.id)), Is.False,
                "Staged events must remain disabled until pacing and editorial review approves registration.");
        }
        ...
        public void CareerStagedBatch_HasValidatedCatalogAndExactChoiceParity()
        {
            var nodes = ReadNodes()
                ...
                .OrderBy(node => node.eventId, StringComparer.Ordinal)
                .ToList();
            var events = StagedEventCatalog.CreateCareerBatch()
                .OrderBy(evt => evt.id, StringComparer.Ordinal)
                .ToList();
        ...
        public void CareerStagedBatch_RemainsOutsideRandomLaunchCatalog()
        {
            ...
            Assert.That(StagedEventCatalog.CreateCareerBatch()
                .Any(evt => launchIds.Contains(evt.id)), Is.False,
                "Staged events must remain disabled until pacing and editorial review approves registration.");
        }
        ...
        public void HealthStagedBatch_HasValidatedCatalogAndExactChoiceParity()
        {
            var nodes = ReadNodes()
                ...
                .OrderBy(node => node.eventId, StringComparer.Ordinal)
                .ToList();
            var events = StagedEventCatalog.CreateHealthBatch()
                .OrderBy(evt => evt.id, StringComparer.Ordinal)
                .ToList();
        ...
        public void HealthStagedBatch_RemainsOutsideRandomLaunchCatalog()
        {
            ...
            Assert.That(StagedEventCatalog.CreateHealthBatch()
                .Any(evt => launchIds.Contains(evt.id)), Is.False,
                "Staged events must remain disabled until pacing and editorial review approves registration.");
        }
        ...
        public void MoneyStagedBatch_HasValidatedCatalogAndExactChoiceParity()
        {
            var nodes = ReadNodes()
                ...
                .OrderBy(node => node.eventId, StringComparer.Ordinal)
                .ToList();
            var events = StagedEventCatalog.CreateMoneyBatch()
                .OrderBy(evt => evt.id, StringComparer.Ordinal)
                .ToList();
        ...
        public void MoneyStagedBatch_RemainsOutsideRandomLaunchCatalog()
        {
            ...
            Assert.That(StagedEventCatalog.CreateMoneyBatch()
                .Any(evt => launchIds.Contains(evt.id)), Is.False,
                "Staged events must remain disabled until pacing and editorial review approves registration.");
        }
