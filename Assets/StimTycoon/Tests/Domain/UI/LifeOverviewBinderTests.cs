...
{
    public sealed class LifeOverviewBinderTests
    {
        [Test]
        public void Constructor_RequiresCompleteNamedContract()
        {
            Assert.Throws<ArgumentNullException>(() => new LifeOverviewBinder(null));
            Assert.That(new LifeOverviewBinder(new VisualElement()).IsValid, Is.False);
            Assert.That(new LifeOverviewBinder(CreateRoot()).IsValid, Is.True);
        }
        ...
        {
            var root = CreateRoot();
            var binder = new LifeOverviewBinder(root);
