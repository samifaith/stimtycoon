...
{
    public sealed class NewLifeBinderTests
    {
        [Test]
        public void Constructor_RequiresCompleteNamedContract()
        {
            Assert.Throws<ArgumentNullException>(() => new NewLifeBinder(null));
            Assert.That(new NewLifeBinder(new VisualElement()).IsValid, Is.False);
            Assert.That(new NewLifeBinder(CreateRoot()).IsValid, Is.True);
        }
        ...
        {
            var root = CreateRoot();
            var binder = new NewLifeBinder(root);
