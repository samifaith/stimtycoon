...
{
    public sealed class BrandComponentGalleryTests
    {
        ...
        public void Gallery_ExercisesInteractiveAndDisabledBrandStates()
        {
            ...
            Assert.That(jellyClaim.ClassListContains("pack-interaction-pop"), Is.True);
        }

        [Test]
        public void Gallery_UsesOnlyResponsiveSafeDesignSystemBindings()
        {
            var theme = File.ReadAllText(ThemePath);
            var galleryStyles = File.ReadAllText(GalleryStylesPath);

            StringAssert.Contains("Assets/StimDesignSystem/Progress/progress-bar.svg", theme);
            StringAssert.Contains("Assets/StimDesignSystem/Buttons/plain-bluegreen.svg", theme);
            StringAssert.Contains("-unity-slice-left: 55", theme);
            StringAssert.Contains("-unity-slice-scale: 0.4px", theme);
            StringAssert.Contains("Assets/StimDesignSystem/Icons/star.png", theme);
            StringAssert.Contains("Assets/StimDesignSystem/Icons", theme);
            StringAssert.DoesNotContain("Space_Exploration_GUI_Kit", theme);
            StringAssert.DoesNotContain("Jelly_UI_Pack", theme);
        ...
        public void PhaseTwoComponents_ExposeStableReusableContracts()
        {
            ...
            Assert.That(actionCard.Q<Button>("action-card-commit").ClassListContains("pack-interaction-pop"), Is.True);
