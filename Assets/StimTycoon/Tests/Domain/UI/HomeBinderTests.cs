...
{
    public sealed class HomeBinderTests
    {
        [Test]
        public void Constructor_RequiresCompleteNamedContract()
        {
            Assert.Throws<ArgumentNullException>(() => new HomeBinder(null));
            Assert.That(new HomeBinder(new VisualElement()).IsValid, Is.False);
            Assert.That(new HomeBinder(CreateRoot()).IsValid, Is.True);
        }
        ...
        {
            var root = CreateRoot();
            var binder = new HomeBinder(root);
        ...
        private static void Render(HomeBinder binder, GameState state, string selectedObjectId = "bookshelf")
        {
            binder.Render(
                state,
                HomeContentCatalog.Get("starter_home"),
                GameSessionService.GetHomeUpgradeRequiredProgress(state.home.upgradeLevel),
