...
namespace StimTycoon.Tests.Domain.Runtime
{
    public sealed class GameSessionServiceTests
    {
        ...
        public void ResolveChoice_IsDeterministicForSameSeedAndStep()
        {
            ...
            var evt = RepresentativeEvents.CreateSalaryNegotiation();
        ...
        public void StartNewLife_CommitsBeforeReplacingActiveLife()
        {
            ...
            var service = new GameSessionService(new InMemoryEventCatalog(), repository);
            var save = NewLifeFactory.Create(
                new NewLifeRequest
                {
                    ...
                    lastName = "Grant",
                    country = "USA",
                    backgroundId = NewLifeFactory.WorkingClassBackground
                },
                "0.1.0",
            ...
            string expectedSummary)
        {
            var service = new GameSessionService(
                new InMemoryEventCatalog(),
                new RecordingSaveRepository());
            var save = CreateValidSave();
            save.state.character.age = startingAge;
            save.state.character.lifeStage = GameSessionService.GetLifeStage(startingAge);
            save.state.education.stage = GameSessionService.GetEducationStage(startingAge);
        ...
        public void ResolveChoice_AppliesStateHistoryRevisionAndAutosave()
        {
            ...
            catalog.Upsert(RepresentativeEvents.CreateSalaryNegotiation());
            var repository = new RecordingSaveRepository();
            var service = new GameSessionService(
                catalog,
                repository,
            ...
            var resolved = service.TryResolveChoice(
                RepresentativeEvents.SalaryNegotiationId,
                "let_it_pass",
                out var summary);
        ...
        public void ResolveChoice_DoesNotMutateActiveSaveWhenCommitFails()
        {
            ...
            var service = new GameSessionService(catalog, repository);
            service.Start(CreateValidSave());
        ...
        public void OutcomeFeedText_FallsBackToTheResolvedResult()
        {
            ...
            Assert.That(
                GameSessionService.BuildOutcomeFeedText(outcome),
        ...
        public void AdvanceMonth_PaysIncrementallyThenEnforcesAnnualCooldown()
        {
            ...
            {
                eventId = RepresentativeEvents.SalaryNegotiationId,
            ...
            Assert.That(nextEvent?.id, Is.EqualTo(RepresentativeEvents.SalaryNegotiationId));
            ...
            Assert.That(repository.LastCommittedSave, Contains.Substring(RepresentativeEvents.SalaryNegotiationId));
        }
        ...
        public void AdvanceMonth_RejectsUnresolvedPendingEvent()
        {
            ...
            var service = new GameSessionService(catalog, repository);
            var save = CreateValidSave();
            save.state.pendingEventId = RepresentativeEvents.SalaryNegotiationId;
            service.Start(save);
            ...
            Assert.That(pendingEvent.id, Is.EqualTo(RepresentativeEvents.SalaryNegotiationId));
            Assert.That(nextEvent, Is.Null);
            Assert.That(summary, Contains.Substring("pending life event"));
            Assert.That(summary, Does.Not.Contain(RepresentativeEvents.SalaryNegotiationId));
            Assert.That(service.ActiveSave.state.character.age, Is.EqualTo(24));
        ...
        {
            var service = new GameSessionService(
                new InMemoryEventCatalog(), new RecordingSaveRepository());
            var save = CreateValidSave();
        ...
        public void AdvanceYear_StopsAtFirstAuthoredEvent()
        {
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(RepresentativeEvents.CreateSalaryNegotiation());
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            var save = CreateValidSave();
            save.state.calendar.monthOfYear = 12;
            service.Start(save);
            ...
            Assert.That(service.ActiveSave.state.pendingEventId,
                Is.EqualTo(RepresentativeEvents.SalaryNegotiationId));
        ...
        {
            var catalog = new InMemoryEventCatalog();
            var reviewEvent = RepresentativeEvents.CreateYearInReview();
            Assert.That(EventValidator.ValidateEvent(reviewEvent).isValid, Is.True);
            ...
            Assert.That(nextEvent?.id, Is.EqualTo(RepresentativeEvents.YearInReviewId));
            Assert.That(service.ActiveSave.state.pendingEventId, Is.EqualTo(RepresentativeEvents.YearInReviewId));
            ...
            Assert.That(GameSessionService.BuildAnnualReviewSummary(service.ActiveSave.state),
        ...
        {
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(RepresentativeEvents.CreateYearInReview());
            var repository = new RecordingSaveRepository();
            var service = new GameSessionService(catalog, repository);
            var save = CreateValidSave();
            save.state.calendar.monthOfYear = 12;
            ...
            Assert.IsTrue(service.TryResolveChoice(
                RepresentativeEvents.YearInReviewId, "build_security", out var firstSummary), firstSummary);
            ...
            Assert.IsFalse(service.TryResolveChoice(
                RepresentativeEvents.YearInReviewId, "build_security", out var duplicateSummary));
        ...
        public void AnnualAccumulator_CapturesSkillsSavingsAndDeterministicallyOrderedHighlights()
        {
            ...
            service.Start(save);
            Assert.IsTrue(service.TryAdvanceMonth(out _, out _));
            service.ActiveSave.state.skills.Add(new SkillState { skillId = "learning", experience = 10 });
            service.ActiveSave.state.finances.savingsMinorUnits = 25000;
            service.ActiveSave.state.lifeFeed.Add(new LifeFeedEntry
            {
            ...
            Assert.That(reviewEvent?.id, Is.EqualTo(RepresentativeEvents.YearInReviewId));
        ...
        public void YearInReview_PersistenceFailureRollsBackRewardAndArchive()
        {
            ...
            Assert.IsFalse(service.TryResolveChoice(
                RepresentativeEvents.YearInReviewId, "build_security", out _));
        ...
        public void YearInReview_ReloadPreservesPendingEntitlementAndSingleClaim()
        {
            ...
            catalog.Upsert(RepresentativeEvents.CreateYearInReview());
            var repository = new RecordingSaveRepository();
            var firstSession = new GameSessionService(catalog, repository);
            var save = CreateValidSave();
            save.state.calendar.monthOfYear = 12;
            firstSession.Start(save);
            Assert.IsTrue(firstSession.TryAdvanceMonth(out _, out _));

            var resumed = new GameSessionService(catalog, repository);
            Assert.IsTrue(resumed.TryLoadLatest(out var loadSummary), loadSummary);
            Assert.That(resumed.ActiveSave.state.pendingEventId,
                Is.EqualTo(RepresentativeEvents.YearInReviewId));
            Assert.IsTrue(resumed.TryGetPendingEvent(out var resumedEvent));
            Assert.That(resumedEvent.id, Is.EqualTo(RepresentativeEvents.YearInReviewId));
            Assert.IsTrue(resumed.TryResolveChoice(
                RepresentativeEvents.YearInReviewId, "nurture_connections", out var claimSummary), claimSummary);
            Assert.IsFalse(resumed.TryResolveChoice(
                RepresentativeEvents.YearInReviewId, "nurture_connections", out _));
        ...
        public void AnnualReviewArchive_RetainsNewestTenCompletedYears()
        {
            ...
            for (var age = 25; age <= 35; age++)
            {
                ...
                service.ActiveSave.state.pendingEventId = RepresentativeEvents.YearInReviewId;
                Assert.IsTrue(service.TryResolveChoice(
                    RepresentativeEvents.YearInReviewId, "invest_in_growth", out var summary), summary);
            }
            ...
            Assert.That(SaveValidator.ValidateSave(service.ActiveSave).isValid, Is.True);
        }
        ...
        public void AdvanceYear_StopsForRequiredSchoolPathDecision()
        {
            ...
            save.state.character.lifeStage = GameSessionService.GetLifeStage(11);
            save.state.education.stage = GameSessionService.GetEducationStage(11);
            save.state.calendar.monthOfYear = 12;
            save.state.career = new CareerState();
            service.Start(save);
        ...
        public void AdvanceMonth_DecrementsAndExpiresTimedStatuses()
        {
            var service = new GameSessionService(
                ...
                new RecordingSaveRepository());
            var save = CreateValidSave();
            save.state.statuses.Add(new StatusState { statusId = "focused", remainingMonths = 2 });
            service.Start(save);
        ...
        public void PerformActivity_StudyAppliesStatsXpCooldownAndAutosave()
        {
            ...
            var performed = service.TryPerformActivity(ActivityType.Study, out var summary);
        ...
        public void PerformActivity_WorkoutAppliesHealthHappinessAndFitnessXp()
        {
            ...
            var performed = service.TryPerformActivity(ActivityType.Workout, out var summary);
        ...
        public void HomeActions_ApplyCostsBenefitsFeedAndIndependentMonthlyCooldowns()
        {
            ...
            Assert.IsTrue(service.TryPerformHomeAction(HomeActionType.Read, out var readSummary), readSummary);
            ...
            Assert.IsFalse(service.TryPerformHomeAction(HomeActionType.Read, out var cooldownSummary));
            Assert.That(cooldownSummary, Does.Contain("already completed"));

            Assert.IsTrue(service.TryPerformHomeAction(HomeActionType.Rest, out var restSummary), restSummary);
            ...
            Assert.IsTrue(service.TryPerformHomeAction(HomeActionType.Read, out readSummary), readSummary);
        }
        ...
        public void HomeAction_RejectsUnaffordableCostAndRollsBackFailedCommit()
        {
            ...
            Assert.IsFalse(service.TryPerformHomeAction(HomeActionType.Read, out var fundsSummary));
            ...
            repository.ShouldCommit = false;
            Assert.IsFalse(service.TryPerformHomeAction(HomeActionType.Maintain, out var failedSummary));
        ...
        public void HomeInventory_IsConsumedBlockedAndRestoredByMaintenance()
        {
            ...
            Assert.IsTrue(service.TryPerformHomeAction(HomeActionType.Read, out var read), read);
            ...
            Assert.IsFalse(service.TryPerformHomeAction(HomeActionType.Read, out var empty));
            Assert.That(empty, Does.Contain("No reading materials"));

            Assert.IsTrue(service.TryPerformHomeAction(HomeActionType.Train, out var train), train);
            ...
            Assert.IsFalse(service.TryPerformHomeAction(HomeActionType.Train, out var unsafeSummary));
            Assert.That(unsafeSummary, Does.Contain("needs maintenance"));

            Assert.IsTrue(service.TryPerformHomeAction(HomeActionType.Maintain, out var maintain), maintain);
        ...
        {
            var starter = HomeContentCatalog.Get("starter_home");
            var valid = HomeContentCatalog.Validate(starter);
            ...
            Assert.That(HomeContentCatalog.GetAction("starter_home", HomeActionType.Read).costMinorUnits,
                Is.EqualTo(500));

            var duplicate = new HomeDefinition
            {
            ...
            Assert.IsFalse(HomeContentCatalog.Validate(duplicate).isValid);
        }
        ...
        {
            var evt = RepresentativeEvents.CreateHomeDeferredMaintenance();
            Assert.IsTrue(EventValidator.ValidateEvent(evt).isValid);
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(evt);
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            var save = CreateValidSave();
            ...
            Assert.That(selected?.id, Is.EqualTo(RepresentativeEvents.HomeDeferredMaintenanceId));
            Assert.That(service.ActiveSave.state.home.condition, Is.EqualTo(19));
        ...
        public void HomeRepairEvent_IsUnavailableToChildrenAndUnlocksAtEighteen()
        {
            ...
            var childSession = new GameSessionService(catalog, new RecordingSaveRepository());
            ...
            var adultSession = new GameSessionService(catalog, new RecordingSaveRepository());
            ...
            Assert.That(adultEvent?.id, Is.EqualTo(RepresentativeEvents.HomeDeferredMaintenanceId));
        }
        ...
        public void ChildHomeMaintenance_UsesCaregiverVariantAndHomeUpgradeRemainsHiddenByAuthority()
        {
            ...
            Assert.IsTrue(service.TryPerformHomeAction(HomeActionType.Maintain, out var maintainSummary),
                maintainSummary);
        ...
        public void AdultFinancialAgencyEvents_BeginAtEighteen()
        {
            var events = new[]
            {
                RepresentativeEvents.CreateHomeDeferredMaintenance(),
                RepresentativeEvents.CreateRandomLoss(),
                RepresentativeEvents.CreateRandomGainRefund(),
                RepresentativeEvents.CreateRandomLossRepair()
            };
        ...
        public void PerformActivity_EnforcesLifeStageAndSupportsChildPlay()
        {
            ...
            Assert.IsFalse(service.TryPerformActivity(ActivityType.Study, out var unavailable));
            Assert.That(unavailable, Does.Contain("not available at age 4"));
            Assert.IsTrue(service.TryPerformActivity(ActivityType.Play, out var summary), summary);
        ...
        public void PerformRelationshipInteraction_AppliesEffectsFeedCooldownAndAutosave()
        {
            ...
            var save = CreateValidSave();
            save.state.character.age = 10;
            save.state.relationships.Add(new RelationshipState
            {
        ...
        public void CompatibleDiscovery_PersistsIdentityContextWarmthStageAndBoundedHistory()
        {
            var repository = new RecordingSaveRepository();
            var service = new GameSessionService(
                new InMemoryEventCatalog(), repository,
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                relationshipId, RelationshipInteractionType.Talk, out var talk), talk);
            ...
            var reloaded = JsonUtility.FromJson<SaveEnvelope>(repository.LastCommittedSave);
            Assert.IsTrue(SaveValidator.ValidateSave(reloaded).isValid);
        ...
        {
            var service = new GameSessionService(new InMemoryEventCatalog(), new RecordingSaveRepository());
            var save = CreateValidSave();
            save.state.relationships.Add(new RelationshipState
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "parent_1", RelationshipInteractionType.Talk, out var firstSummary), firstSummary);
            Assert.IsFalse(service.TryPerformRelationshipInteraction(
                "parent_1", RelationshipInteractionType.SpendTime, out var duplicateSummary));
            Assert.That(duplicateSummary, Does.Contain("already spent focused time"));
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "parent_2", RelationshipInteractionType.SpendTime, out var secondSummary), secondSummary);
        }

        [TestCase(7, RelationshipInteractionType.Argue, false)]
        [TestCase(8, RelationshipInteractionType.Argue, true)]
        [TestCase(12, RelationshipInteractionType.PlayTogether, true)]
        [TestCase(13, RelationshipInteractionType.PlayTogether, false)]
        [TestCase(17, RelationshipInteractionType.AskForHelp, true)]
        [TestCase(18, RelationshipInteractionType.AskForHelp, false)]
        [TestCase(17, RelationshipInteractionType.AskOnDate, false)]
        [TestCase(18, RelationshipInteractionType.AskOnDate, true)]
        [TestCase(20, RelationshipInteractionType.Commit, false)]
        [TestCase(21, RelationshipInteractionType.Commit, true)]
        [TestCase(17, RelationshipInteractionType.DateNight, false)]
        [TestCase(18, RelationshipInteractionType.DateNight, true)]
        [TestCase(17, RelationshipInteractionType.Separate, false)]
        [TestCase(18, RelationshipInteractionType.Separate, true)]
        [TestCase(17, RelationshipInteractionType.BreakUp, false)]
        [TestCase(18, RelationshipInteractionType.BreakUp, true)]
        [TestCase(70, RelationshipInteractionType.Talk, true)]
        ...
        {
            Assert.That(
                GameSessionService.IsRelationshipInteractionAgeAppropriate(interactionType, age),
                Is.EqualTo(expected));
        }
        ...
        public void AdultFriendship_CanBecomePartnershipAndEndWithPersistentFeedHistory()
        {
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "friend_adult_1", RelationshipInteractionType.AskOnDate, out var dateSummary), dateSummary);
            ...
            Assert.IsFalse(service.TryPerformRelationshipInteraction(
                "friend_adult_1", RelationshipInteractionType.Commit, out var cooldownSummary));
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "friend_adult_1", RelationshipInteractionType.Commit, out var commitSummary), commitSummary);
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "friend_adult_1", RelationshipInteractionType.BreakUp, out var breakupSummary), breakupSummary);
        ...
        public void RelationshipLifecycle_RequiresAndPersistsFriendshipRomanceSeparationAndRecoveryStages()
        {
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "compatible_1", RelationshipInteractionType.DeepenFriendship, out var deepen), deepen);
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "compatible_1", RelationshipInteractionType.AskOnDate, out var date), date);
            Assert.That(service.ActiveSave.state.relationships[0].relationshipStage, Is.EqualTo("dating"));

            AdvanceAndAssert(service);
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "compatible_1", RelationshipInteractionType.DateNight, out var dateNight), dateNight);
            Assert.That(service.ActiveSave.state.relationships[0].relationshipStage, Is.EqualTo("romantic_growth"));

            AdvanceAndAssert(service);
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "compatible_1", RelationshipInteractionType.Commit, out var commit), commit);
            Assert.That(service.ActiveSave.state.relationships[0].relationshipType, Is.EqualTo("partner"));

            AdvanceAndAssert(service);
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "compatible_1", RelationshipInteractionType.Separate, out var separate), separate);
            Assert.That(service.ActiveSave.state.relationships[0].relationshipStage, Is.EqualTo("separated"));

            AdvanceAndAssert(service);
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "compatible_1", RelationshipInteractionType.Recover, out var recover), recover);
            ...
            Assert.That(JsonUtility.FromJson<SaveEnvelope>(repository.LastCommittedSave)
        ...
        public void FamilyPlanning_RequiresMutualDiscussionAndPregnancyCreatesDurableChild()
        {
            ...
            Assert.IsFalse(service.TryChooseFamilyPlanning(
                "partner_1", FamilyPlanningAction.TryForChild, out var consentRequired));
            Assert.That(consentRequired, Does.Contain("Both partners"));
            Assert.IsTrue(service.TryChooseFamilyPlanning(
                "partner_1", FamilyPlanningAction.Discuss, out var discussed), discussed);
            Assert.IsTrue(service.ActiveSave.state.family.partnerConsent);

            AdvanceAndAssert(service);
            Assert.IsTrue(service.TryChooseFamilyPlanning(
                "partner_1", FamilyPlanningAction.TryForChild, out var started), started);
            ...
            Assert.IsTrue(SaveValidator.ValidateSave(
                JsonUtility.FromJson<SaveEnvelope>(repository.LastCommittedSave)).isValid);
        }
        ...
        public void Adoption_IsAtomicTimedAndAddsOngoingChildExpenses()
        {
            ...
            Assert.IsTrue(service.TryChooseFamilyPlanning(
                "spouse_1", FamilyPlanningAction.PursueAdoption, out var adoption), adoption);
            Assert.That(service.ActiveSave.state.finances.cashMinorUnits, Is.EqualTo(50000));
        ...
        public void ParentingActions_UpdateChildDevelopmentHistoryAndMonthlyCooldown()
        {
            ...
            Assert.IsTrue(service.TryPerformParentingAction(
                "child_1", ParentingAction.Teach, out var teach), teach);
            var child = service.ActiveSave.state.family.children[0];
            ...
            Assert.IsFalse(service.TryPerformParentingAction(
                "child_1", ParentingAction.QualityTime, out var cooldown));
            Assert.That(cooldown, Does.Contain("already chose focused parenting"));

            AdvanceAndAssert(service);
            Assert.IsTrue(service.TryPerformParentingAction(
                "child_1", ParentingAction.SupportNeeds, out var support), support);
        ...
        public void AdultChildRole_IsProtectedFromFriendshipAndRomanceStageConversion()
        {
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "adult_child_1", RelationshipInteractionType.Talk, out var summary), summary);
            ...
            Assert.IsFalse(service.TryPerformRelationshipInteraction(
                "adult_child_1", RelationshipInteractionType.AskOnDate, out var dating));
        ...
        public void PartnerSeparation_AssignsSharedCustodyWithoutDeletingChildHistory()
        {
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "partner_1", RelationshipInteractionType.Separate, out var summary), summary);
        ...
        public void SharedCustodyAndChildHistory_SurviveReloadAndRemainPlayable()
        {
            ...
            service.Start(save);
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "partner_1", RelationshipInteractionType.Separate, out var separation), separation);

            var reloaded = new GameSessionService(new InMemoryEventCatalog(), repository);
            reloaded.Start(JsonUtility.FromJson<SaveEnvelope>(repository.LastCommittedSave));
            ...
            Assert.IsTrue(reloaded.TryPerformParentingAction(
                "child_1", ParentingAction.QualityTime, out var parenting), parenting);
        ...
        public void EndedLife_BlocksParentingWithoutChangingChildOrHistory()
        {
            ...
            Assert.IsFalse(service.TryPerformParentingAction(
                "child_1", ParentingAction.QualityTime, out var summary));
        ...
        public void StartNewLife_DoesNotCarryFamilyOrChildRelationshipsAcrossLifeBoundary()
        {
            ...
            var newLife = NewLifeFactory.Create(
                new NewLifeRequest(), "0.1.0", new System.DateTime(2026, 7, 14, 12, 0, 0, System.DateTimeKind.Utc), 404);
        ...
        {
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(RepresentativeEvents.CreateComingOfAgeGender());
            catalog.Upsert(RepresentativeEvents.CreateComingOfAgeOrientation());
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            var save = CreateValidSave();
            save.state.character.age = 15;
            save.state.character.lifeStage = GameSessionService.GetLifeStage(15);
            ...
            Assert.That(genderEvent?.id, Is.EqualTo(RepresentativeEvents.ComingOfAgeGenderId));
            Assert.IsTrue(service.TryResolveChoice(
            ...
            Assert.That(service.ActiveSave.state.scheduledEvents.Exists(
                record => record.eventId == RepresentativeEvents.ComingOfAgeOrientationId), Is.True);

            Event orientationEvent = null;
            for (var month = 0; month < 12; month++)
            {
                Assert.IsTrue(service.TryAdvanceMonth(out orientationEvent, out var advanceSummary), advanceSummary);
            }

            Assert.That(orientationEvent?.id, Is.EqualTo(RepresentativeEvents.ComingOfAgeOrientationId));
            Assert.IsTrue(service.TryResolveChoice(
        ...
        {
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(RepresentativeEvents.CreatePromInvitation());
            catalog.Upsert(RepresentativeEvents.CreateFirstKiss());
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            var save = CreateValidSave();
            ...
            Assert.IsTrue(service.TryResolveChoice(
                RepresentativeEvents.PromInvitationId,
            ...
            Assert.That(service.ActiveSave.state.scheduledEvents.Exists(
                record => record.eventId == RepresentativeEvents.FirstKissId), Is.True);

            service.ActiveSave.state.character.age = 19;
            Assert.IsTrue(service.TryResolveChoice(
                RepresentativeEvents.FirstKissId,
                "wait_to_kiss",
        ...
        {
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(RepresentativeEvents.CreateProposal());
            catalog.Upsert(RepresentativeEvents.CreateWedding());
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            var save = CreateValidSave();
            ...
            Assert.IsTrue(service.TryResolveChoice(
                RepresentativeEvents.ProposalId, "propose_marriage", out var proposalSummary), proposalSummary);
            ...
            Assert.That(service.ActiveSave.state.finances.cashMinorUnits, Is.EqualTo(50000));
            Assert.That(service.ActiveSave.state.scheduledEvents.Exists(
                record => record.eventId == RepresentativeEvents.WeddingId), Is.True);

            Assert.IsTrue(service.TryResolveChoice(
                RepresentativeEvents.WeddingId, "get_married", out var weddingSummary), weddingSummary);
        ...
        public void WeddingFinancialImpact_CanBePositiveOrNegativeBasedOnLifeFactors()
        {
            ...
            modestState.relationships.Add(new RelationshipState
            {
                relationshipId = "school_peer_primary",
                relationshipType = "engaged",
                value = 95
            ...
            affluentState.relationships.Add(new RelationshipState
            {
                relationshipId = "school_peer_primary",
                relationshipType = "engaged",
                value = 80
            ...
            Assert.That(GameSessionService.CalculateEventFinancialImpact(
                RepresentativeEvents.WeddingId, "get_married", modestState), Is.GreaterThan(0));
            Assert.That(GameSessionService.CalculateEventFinancialImpact(
                RepresentativeEvents.WeddingId, "get_married", affluentState), Is.LessThan(0));
        }

        [TestCase(RepresentativeEvents.HealthBurnoutId, "take_a_break")]
        [TestCase(RepresentativeEvents.PeerTrustConflictId, "keep_confidence")]
        [TestCase(RepresentativeEvents.ComingOfAgeGenderId, "identify_nonbinary")]
        [TestCase(RepresentativeEvents.FirstKissId, "share_first_kiss")]
        [TestCase(RepresentativeEvents.ProposalId, "end_partnership")]
        public void NonFinancialEventChoices_HaveNoImplicitFinancialImpact(string eventId, string choiceId)
        {
            ...
            Assert.That(
                GameSessionService.CalculateEventFinancialImpact(eventId, choiceId, state),
                Is.Zero);
        }
        ...
        public void Proposal_RejectsWeakOrUnderagePartnership()
        {
            ...
            Assert.IsFalse(service.TryResolveChoice(
                RepresentativeEvents.ProposalId, "propose_marriage", out var summary));
        ...
        public void CostedEvent_OffersCreditWhenCashIsInsufficientAndLineIsAvailable()
        {
            ...
            Assert.IsFalse(service.TryResolveChoice(
                RepresentativeEvents.ProposalId, "propose_marriage", out var cashFailure));
            Assert.That(cashFailure, Does.Contain("more cash").And.Contain("credit option"));
            Assert.That(service.ActiveSave.state.relationships[0].relationshipType, Is.EqualTo("partner"));

            Assert.IsTrue(service.TryResolveChoice(
                RepresentativeEvents.ProposalId,
                "propose_marriage",
        ...
        {
            var evt = RepresentativeEvents.CreateRandomLoss();
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(evt);
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            var save = CreateValidSave();
            ...
            Assert.That(GameSessionService.CalculateChoicePotentialCost(
            ...
            Assert.IsTrue(service.TryResolveChoice(
                evt.id, "handle_it_now", PaymentMethod.Credit, out var creditSummary), creditSummary);
        ...
        {
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(RepresentativeEvents.CreateMarriageCrossroads());
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            var save = CreateValidSave();
            ...
            Assert.IsTrue(service.TryResolveChoice(
                RepresentativeEvents.MarriageCrossroadsId, "divorce", out var divorceSummary), divorceSummary);
            Assert.That(service.ActiveSave.state.relationships[0].relationshipType, Is.EqualTo("ex_partner"));
        ...
        public void MarriageCrossroads_RejectsHealthyMarriage()
        {
            ...
            Assert.IsFalse(service.TryResolveChoice(
                RepresentativeEvents.MarriageCrossroadsId, "divorce", out var summary));
        ...
        public void PerformRelationshipInteraction_DoesNotMutateWhenCommitFails()
        {
            ...
            var performed = service.TryPerformRelationshipInteraction(
                "parent_1", RelationshipInteractionType.Talk, out _);
        ...
        public void FamilyMovie_ImprovesHouseholdAndFamilyRelationshipsAndChargesTickets()
        {
            ...
            Assert.IsTrue(service.TryPerformActivity(
                ActivityType.FamilyMovie, out var summary), summary);
        ...
        public void FamilyMovieCredit_RequiresCreditLineAndAddsOnlyFixedTicketDebt()
        {
            ...
            Assert.IsFalse(service.TryPerformActivity(
                ActivityType.FamilyMovie, out var cashSummary));
            Assert.That(cashSummary, Does.Contain("costs $45").And.Contain("credit option"));
            Assert.IsTrue(service.TryPerformActivity(
                ActivityType.FamilyMovieCredit, out var creditSummary), creditSummary);
        ...
        public void HouseholdCreditApr_RespondsToIncomeStabilityCohesionAndDebtLoad()
        {
            ...
            Assert.That(GameSessionService.CalculateHouseholdCreditAprBasisPoints(strong), Is.EqualTo(800));
            Assert.That(GameSessionService.CalculateHouseholdCreditAprBasisPoints(risky), Is.EqualTo(2999));
        }
        ...
        {
            Assert.That(GameSessionService.GetSkillLevel(experience), Is.EqualTo(expectedLevel));
        }

        [TestCase(0, CoreStatBand.Critical)]
        [TestCase(19, CoreStatBand.Critical)]
        [TestCase(20, CoreStatBand.Low)]
        [TestCase(39, CoreStatBand.Low)]
        [TestCase(40, CoreStatBand.Stable)]
        [TestCase(59, CoreStatBand.Stable)]
        [TestCase(60, CoreStatBand.Strong)]
        [TestCase(79, CoreStatBand.Strong)]
        [TestCase(80, CoreStatBand.Exceptional)]
        [TestCase(94, CoreStatBand.Exceptional)]
        [TestCase(95, CoreStatBand.Peak)]
        [TestCase(100, CoreStatBand.Peak)]
        public void CoreStatBands_UseDocumentedExactBoundaries(
            int value, CoreStatBand expectedBand)
        {
            Assert.That(ProgressionStandards.GetCoreStatBand(value), Is.EqualTo(expectedBand));
            Assert.That(ProgressionStandards.MaximumMainPathCoreStatRequirement,
                Is.EqualTo(ProgressionStandards.StrongCoreStatStartsAt));
        }
        ...
        {
            Assert.That(GameSessionService.GetExperienceForSkillLevel(level),
        ...
        {
            Assert.That(EducationActionService.CertificateQualificationExperience,
                Is.EqualTo(ProgressionStandards.CertificateQualificationExperience));
            Assert.That(EducationActionService.DiplomaQualificationExperience,
                Is.EqualTo(ProgressionStandards.DiplomaQualificationExperience));
            Assert.That(EducationActionService.AdvancedQualificationExperience,
                Is.EqualTo(ProgressionStandards.AdvancedQualificationExperience));
            Assert.That(GameSessionService.IndexInvestmentMinimumAge,
                Is.EqualTo(ProgressionStandards.IndexInvestmentMinimumAge));
            Assert.That(GameSessionService.IndexInvestmentMinimumSmarts,
                Is.EqualTo(ProgressionStandards.IndexInvestmentMinimumSmarts));

            Assert.IsTrue(CareerCatalog.TryGetIndustry(
                CareerCatalog.FinanceIndustryId, out var finance));
            Assert.That(finance.roles.Take(3).Select(role => role.promotionProgressRequired),
                Is.EqualTo(new[]
                {
                    ProgressionStandards.FirstCareerPromotionProgress,
                    ProgressionStandards.SecondCareerPromotionProgress,
                    ProgressionStandards.ThirdCareerPromotionProgress
                }));
            Assert.That(ProgressionStandards.GetBusinessUpgradeProgressRequired(1), Is.EqualTo(25));
            Assert.That(ProgressionStandards.GetBusinessUpgradeProgressRequired(2), Is.EqualTo(50));
            Assert.That(ProgressionStandards.GetBusinessUpgradeProgressRequired(3), Is.EqualTo(75));

            var educationState = CreateValidSave().state;
            educationState.character.age = 14;
            educationState.skills.Add(new SkillState { skillId = "learning", experience = 150 });
            var educationActions = EducationActionService.GetDefinitions(educationState);
            ...
            Assert.That(readExperience, Is.InRange(
                ProgressionStandards.RoutineSkillExperienceMinimum,
                ProgressionStandards.RoutineSkillExperienceMaximum));
            foreach (var action in educationActions.Where(action => action.id != "education.read"))
            {
                ...
                Assert.That(experience, Is.InRange(
                    ProgressionStandards.CommittedSkillExperienceMinimum,
                    ProgressionStandards.CommittedSkillExperienceMaximum), action.id);
            }

            var service = new GameSessionService(
                new InMemoryEventCatalog(), new RecordingSaveRepository());
            service.Start(CreateValidSave());
            Assert.IsTrue(service.TryPerformActivity(ActivityType.Rest, out var summary), summary);
            var goals = service.GetGoals();
            Assert.That(goals, Is.Not.Empty);
            Assert.That(goals.All(goal => ProgressionStandards.IsGoalRewardWithinDocumentedBand(
                goal.category, goal.rewardMinorUnits)), Is.True);
            Assert.That(goals.Single(goal => goal.category == "daily").rewardMinorUnits,
                Is.EqualTo(ProgressionStandards.DailyGoalRewardMinorUnits));
            Assert.That(goals.Single(goal => goal.category == "main").rewardMinorUnits,
                Is.EqualTo(ProgressionStandards.MainGoalRewardMinorUnits));
            Assert.That(goals.Single(goal => goal.category == "life").rewardMinorUnits,
                Is.EqualTo(ProgressionStandards.LifeGoalRewardMinorUnits));
        }
        ...
        public void EducationActions_ExposeVisibleAgeAndLevelRequirements()
        {
            ...
            Assert.IsTrue(GameSessionService.TryGetEducationActionRequirement(
                state, EducationActionType.Read, out var readRequirement), readRequirement);
            Assert.IsFalse(GameSessionService.TryGetEducationActionRequirement(
                state, EducationActionType.StudyGroup, out var groupRequirement));
            Assert.That(groupRequirement, Does.Contain("Learning Level 2"));
            Assert.IsFalse(GameSessionService.TryGetEducationActionRequirement(
                state, EducationActionType.AdvancedProject, out var projectRequirement));
            Assert.That(projectRequirement, Does.Contain("age 14"));

            state.skills.Add(new SkillState { skillId = "learning", experience = 150 });
            state.character.age = 14;
            Assert.IsTrue(GameSessionService.TryGetEducationActionRequirement(
                state, EducationActionType.AdvancedProject, out var unlockedRequirement), unlockedRequirement);
        }
        ...
        public void EducationActionDefinitions_ExposeStableIdsStatesAndSignedPreviews()
        {
            ...
            Assert.That(definitions, Has.Some.Matches<ActionDefinition>(definition =>
                definition.id == "education.read" &&
                definition.destination == ActionDestination.Education &&
            ...
            Assert.That(definitions, Has.Some.Matches<ActionDefinition>(definition =>
                ...
                definition.state == ActionState.Locked &&
        ...
        public void PerformEducationAction_AppliesXpStatsFeedCooldownAndAutosave()
        {
            ...
            save.state.skills.Add(new SkillState { skillId = "learning", experience = 45 });
            service.Start(save);

            Assert.IsTrue(service.TryPerformEducationAction(
                EducationActionType.Homework, out var summary), summary);

            Assert.That(summary, Does.Contain("Learning XP +18").And.Contain("Learning Level +1"));
            Assert.That(GameSessionService.GetSkillExperience(
            ...
            Assert.IsFalse(service.TryPerformEducationAction(
                EducationActionType.Read, out var cooldownSummary));
        ...
        public void PerformEducationAction_WhenAutosaveFails_RollsBackCandidate()
        {
            ...
            Assert.IsFalse(service.TryPerformEducationAction(
                EducationActionType.Read, out var summary));
            ...
            Assert.That(service.ActiveSave.state.statuses.Exists(
                status => status.statusId == EducationActionService.MonthlyCooldownStatusId), Is.False);
            Assert.That(repository.CommitCount, Is.EqualTo(1));
        }
        ...
        {
            var disciplines = EducationDisciplineCatalog.GetAll();
            ...
            Assert.That(disciplines, Has.Some.Matches<EducationDisciplineDefinition>(item =>
        ...
        public void ChooseStudyTrack_DeductsAuthoredCostPersistsAndWritesFeed()
        {
            ...
            var save = CreateValidSave();
            save.state.character.age = 15;
            save.state.education = new EducationState { stage = "high_school" };
            save.state.finances.cashMinorUnits = 10000;
            service.Start(save);

            Assert.IsTrue(service.TryChooseStudyTrack(StudyTrack.Vocational, out var summary), summary);
            ...
            Assert.That(service.ActiveSave.state.lifeFeed, Has.Some.Matches<LifeFeedEntry>(entry =>
        ...
        public void ChooseStudyTrack_RejectsAgeFundsAndDuplicateWithoutMutation()
        {
            ...
            save.state.education = new EducationState { stage = "middle_school" };
            save.state.finances.cashMinorUnits = 1000;
            service.Start(save);

            Assert.IsFalse(service.TryChooseStudyTrack(StudyTrack.General, out var ageSummary));
            Assert.That(ageSummary, Does.Contain("ages 14 through 17"));

            service.ActiveSave.state.character.age = 15;
            Assert.IsFalse(service.TryChooseStudyTrack(StudyTrack.Academic, out var fundsSummary));
            ...
            Assert.IsTrue(service.TryChooseStudyTrack(StudyTrack.General, out var firstSummary), firstSummary);
            Assert.IsFalse(service.TryChooseStudyTrack(StudyTrack.Vocational, out var duplicateSummary));
        ...
        public void ChooseStudyTrack_WhenAutosaveFails_RollsBackCostAndSelection()
        {
            ...
            Assert.IsFalse(service.TryChooseStudyTrack(StudyTrack.Academic, out var summary));
        ...
        public void StudySession_AppliesDifficultyTradeoffQualificationTierAndCooldown()
        {
            ...
            var service = new GameSessionService(
                new InMemoryEventCatalog(), repository, utcNow: () => now);
            var save = CreateValidSave();
            save.state.character.age = 15;
            ...
            Assert.IsTrue(service.TryStartStudySession(
                StudyDifficulty.Hard, "hard-study-1", out var started), started);
            ...
            Assert.That(service.ActiveSave.state.statuses, Has.Some.Matches<StatusState>(status =>
                status.statusId == EducationActionService.MonthlyCooldownStatusId));
            Assert.IsFalse(service.TryStartStudySession(
                StudyDifficulty.Easy, "easy-study-1", out var cooldown));
        ...
        {
            Assert.That(EducationActionService.GetQualificationTier(experience),
                Is.EqualTo(expectedTier));
            Assert.That(EducationActionService.GetNextQualificationTierAt(experience),
        ...
        public void EasyStudyRoute_ReachesAdvancedQualificationWithinTeenStudyWindow()
        {
            var now = DateTimeOffset.Parse("2026-07-15T12:00:00Z");
            var service = new GameSessionService(
                new InMemoryEventCatalog(), new RecordingSaveRepository(), utcNow: () => now);
            var save = CreateValidSave();
            save.state.character.age = 15;
            ...
            for (var month = 0; month < 25; month++)
            {
                ...
                Assert.IsTrue(service.TryStartStudySession(
                    StudyDifficulty.Easy, instanceId, out var startSummary), startSummary);
            ...
            Assert.That(service.ActiveSave.state.education.qualificationExperience,
                Is.EqualTo(EducationActionService.AdvancedQualificationExperience));
            Assert.That(EducationActionService.GetQualificationTier(
        ...
        public void EducationActionRequest_IsIdempotentAcrossRepeatedSubmission()
        {
            ...
            var request = new ActionRequest(
                EducationActionService.GetActionId(EducationActionType.Read),
                "education-request-1");

            Assert.IsTrue(service.TryPerformEducationAction(
                EducationActionType.Read, request, out var firstSummary), firstSummary);
            ...
            var reloadedService = new GameSessionService(
                new InMemoryEventCatalog(), repository);
            ...
            Assert.IsFalse(reloadedService.TryPerformEducationAction(
                EducationActionType.Read, request, out var repeatedSummary));
        ...
        public void TimedAction_StartsInProgressAndCannotBeClaimedEarly()
        {
            ...
            Assert.IsTrue(service.TryStartAction(
                definition, new ActionRequest(definition.id, "timed-1"), out var startSummary), startSummary);
        ...
        public void TimedStudySession_GrantsRewardOnceOnlyAfterCompletion()
        {
            ...
            Assert.IsTrue(service.TryStartStudySession(
                StudyDifficulty.Medium, "focused-study-1", out var started), started);
        ...
        public void TimedAction_ReconcilesAfterReloadAndCanOnlyBeClaimedOnce()
        {
            var repository = new RecordingSaveRepository();
            var now = DateTimeOffset.Parse("2026-07-14T20:00:00Z");
            var firstSession = new GameSessionService(
                new InMemoryEventCatalog(), repository, utcNow: () => now);
            ...
            Assert.IsTrue(firstSession.TryStartAction(
                definition, new ActionRequest(definition.id, "timed-2"), out var startSummary), startSummary);

            now = now.AddSeconds(61);
            var reloadedSession = new GameSessionService(
                new InMemoryEventCatalog(), repository, utcNow: () => now);
        ...
        public void TimedAction_PauseSurvivesReloadAndResumeUsesRemainingTime()
        {
            ...
            var first = new GameSessionService(
                new InMemoryEventCatalog(), repository, utcNow: () => now);
            first.Start(CreateValidSave());
            var definition = new ActionDefinition
            {
                id = "home.reading-timer", title = "Read", state = ActionState.Ready,
                durationSeconds = 60
            };
            Assert.IsTrue(first.TryStartAction(
                definition, new ActionRequest(definition.id, "home-timer-1"), out _));
            ...
            now = now.AddHours(2);
            var reloaded = new GameSessionService(
                new InMemoryEventCatalog(), repository, utcNow: () => now);
        ...
        public void TimedAction_ExpiresAfterAuthoredClaimWindow()
        {
            ...
            var definition = new ActionDefinition
            {
                id = "home.short-timer", title = "Short timer", state = ActionState.Ready,
                durationSeconds = 60, claimWindowSeconds = 10
            };
            Assert.IsTrue(service.TryStartAction(
                definition, new ActionRequest(definition.id, "expiring-1"), out _));
        ...
        public void TimedAction_PrunesTerminalHistoryAndBoundsActiveEntries()
        {
            var service = new GameSessionService(
                new InMemoryEventCatalog(), new RecordingSaveRepository());
            var save = CreateValidSave();
            for (var index = 0; index < ActionProgressState.MaxEntries; index++)
            ...
            service.Start(save);
            var definition = new ActionDefinition
                { id = "home.new", title = "New", state = ActionState.Ready, durationSeconds = 30 };
            Assert.IsTrue(service.TryStartAction(
                definition, new ActionRequest(definition.id, "new-1"), out var started), started);
            Assert.That(service.ActiveSave.state.actionProgress, Has.Count.EqualTo(ActionProgressState.MaxEntries));
            Assert.That(service.ActiveSave.state.actionProgress, Has.Some.Matches<ActionProgressState>(item => item.instanceId == "new-1"));
        }
        ...
        public void StudyMatch_SameSeedAndInstanceGenerateIdenticalPlayableBoards()
        {
            var first = new GameSessionService(
                new InMemoryEventCatalog(), new RecordingSaveRepository());
            var second = new GameSessionService(
                new InMemoryEventCatalog(), new RecordingSaveRepository());
            first.Start(CreateValidSave());
            ...
            Assert.IsTrue(MatchEngine.HasLegalMove(first.ActiveSave.state.matchSession.board, 8, 8));
        ...
        public void StudyMatch_RejectsInvalidSwapAndResolvesLegalSwapDeterministically()
        {
            ...
            Assert.IsTrue(MatchEngine.HasLegalMove(service.ActiveSave.state.matchSession.board, 8, 8));
        }
        ...
        public void StudyMatch_PauseSurvivesReloadAndTimeoutReconcilesAfterResume()
        {
            ...
            var reloadedSave = JsonUtility.FromJson<SaveEnvelope>(JsonUtility.ToJson(service.ActiveSave));
            now = now.AddDays(2);
            var resumed = new GameSessionService(
                new InMemoryEventCatalog(), new RecordingSaveRepository(), utcNow: () => now);
        ...
        public void MatchThemesShareEngineAndClaimedSessionEnforcesCooldown()
        {
            foreach (var theme in new[] { "study_match", "shift_match", "legacy_gems" })
            {
                ...
                Assert.That(MatchEngine.HasLegalMove(
                    service.ActiveSave.state.matchSession.board, 8, 8), Is.True);
            }

            var cooldownService = new GameSessionService(
                new InMemoryEventCatalog(), new RecordingSaveRepository());
        ...
        public void CareerApplication_UsesSelectedTrackQualificationAsVisiblePrerequisite()
        {
            ...
            Assert.IsFalse(GameSessionService.TryGetCareerActionRequirement(
                service.ActiveSave.state, CareerActionType.Apply, out var requirement));
            Assert.That(requirement, Does.Contain("Certificate qualification (50 XP)"));
            Assert.IsFalse(service.TryPerformCareerAction(CareerActionType.Apply, out var lockedSummary));
            Assert.That(lockedSummary, Is.EqualTo(requirement));

            service.ActiveSave.state.education.qualificationExperience = 50;
            Assert.IsTrue(service.TryPerformCareerAction(CareerActionType.Apply, out var summary), summary);
        }

        [Test]
        public void CareerIndustries_ExposeDistinctStableLaddersAndRequirements()
        {
            var industries = CareerCatalog.GetIndustries();
            ...
            Assert.That(industries.Select(industry => industry.industryId), Is.EquivalentTo(new[]
            {
                ...
                CareerCatalog.HealthcareIndustryId,
                CareerCatalog.SkilledTradesIndustryId
            }));
        ...
        public void HealthcareApplication_RequiresAcademicQualificationAndLearningThenPersistsIndustry()
        {
            ...
            save.state.skills.Add(new SkillState { skillId = "learning", experience = 50 });
            service.Start(save);

            Assert.IsFalse(service.TryApplyForCareer(
                CareerCatalog.HealthcareIndustryId, out var locked));
            ...
            Assert.IsTrue(service.TryApplyForCareer(
                CareerCatalog.HealthcareIndustryId, out var applied), applied);
            Assert.That(service.ActiveSave.state.career.pendingIndustryId,
                Is.EqualTo(CareerCatalog.HealthcareIndustryId));
            AdvanceAndAssert(service);
            Assert.IsTrue(service.TryPerformCareerAction(
                CareerActionType.Interview, out var interview), interview);
            ...
            Assert.That(JsonUtility.FromJson<SaveEnvelope>(repository.LastCommittedSave)
                .state.career.industryId, Is.EqualTo(CareerCatalog.HealthcareIndustryId));
        }
        ...
        public void SkilledTradesPromotion_UsesIndustrySpecificThresholdAndRole()
        {
            ...
            save.state.career = new CareerState
            {
                industryId = CareerCatalog.SkilledTradesIndustryId,
            ...
            Assert.IsTrue(service.TryPerformCareerAction(
                CareerActionType.AskForPromotion, out var summary), summary);
        ...
        public void InterviewRejection_IsDeterministicAndUnlocksTransactionalRetraining()
        {
            ...
            Assert.IsTrue(service.TryApplyForCareer(
                CareerCatalog.FinanceIndustryId, out var application), application);
            AdvanceAndAssert(service);
            Assert.IsTrue(service.TryPerformCareerAction(
                CareerActionType.Interview, out var interview), interview);
            ...
            Assert.IsTrue(service.TryPerformCareerAction(
                CareerActionType.Retrain, out var retraining), retraining);
        ...
        {
            Assert.IsTrue(CareerCatalog.TryGetIndustry(
                CareerCatalog.HealthcareIndustryId, out var industry));
            ...
            developed.skills.Add(new SkillState { skillId = "professional", experience = 150 });

            var baselineChance = GameSessionService.CalculateInterviewSuccessChance(baseline, industry);
            var developedChance = GameSessionService.CalculateInterviewSuccessChance(developed, industry);
        ...
        public void Retraining_IsUnavailableWhileEmployed()
        {
            ...
            Assert.IsFalse(GameSessionService.TryGetCareerActionRequirement(
                state, CareerActionType.Retrain, out var requirement));
        ...
        public void RepeatedLowPerformanceWarning_CanCauseDeterministicFiringAndUnemployment()
        {
            ...
            save.state.career = new CareerState
            {
                industryId = CareerCatalog.FinanceIndustryId,
        ...
        public void LocalServicesBusiness_StartUpgradeAndSaleAreTransactional()
        {
            ...
            var save = CreateValidSave();
            save.state.finances.cashMinorUnits = 1000000;
            save.state.skills.Add(new SkillState { skillId = "professional", experience = 50 });
            service.Start(save);

            Assert.IsTrue(service.TryPerformBusinessAction(BusinessActionType.Start, out var started), started);
            Assert.That(service.ActiveSave.state.finances.cashMinorUnits, Is.EqualTo(900000));
            AdvanceAndAssert(service);
            Assert.IsTrue(service.TryPerformBusinessAction(BusinessActionType.Work, out var worked), worked);
            ...
            service.ActiveSave.state.business.operatingProgress =
                ProgressionStandards.GetBusinessUpgradeProgressRequired(1) - 1;
            Assert.IsFalse(service.TryPerformBusinessAction(
                BusinessActionType.Upgrade, out var progressLocked));
            Assert.That(progressLocked, Does.Contain(
                $"{ProgressionStandards.GetBusinessUpgradeProgressRequired(1)} operating progress"));
            service.ActiveSave.state.business.operatingProgress =
                ProgressionStandards.GetBusinessUpgradeProgressRequired(1);
            Assert.IsTrue(service.TryPerformBusinessAction(
                BusinessActionType.Upgrade, out var upgraded), upgraded);
            ...
            Assert.IsTrue(service.TryPerformBusinessAction(BusinessActionType.Sell, out var sold), sold);
        ...
        public void BusinessStartup_EnforcesSkillFundsAndPersistenceRollback()
        {
            ...
            var save = CreateValidSave();
            service.Start(save);
            Assert.IsFalse(service.TryPerformBusinessAction(BusinessActionType.Start, out var skillLocked));
            Assert.That(skillLocked, Does.Contain("Professional Level 2"));
            service.ActiveSave.state.skills.Add(new SkillState { skillId = "professional", experience = 50 });
            service.ActiveSave.state.finances.cashMinorUnits = 99999;
            Assert.IsFalse(service.TryPerformBusinessAction(BusinessActionType.Start, out var fundsLocked));
            Assert.That(fundsLocked, Does.Contain("costs $1,000"));
            service.ActiveSave.state.finances.cashMinorUnits = 100000;
            repository.ShouldCommit = false;
            Assert.IsFalse(service.TryPerformBusinessAction(BusinessActionType.Start, out _));
        ...
        public void Business_ClosesAfterThirdLossAndBoundsLedger()
        {
            ...
            for (var index = 0; index < 60; index++)
                save.state.business.ledger.Add(new BusinessLedgerEntry
                {
        ...
        public void BusinessValuation_UsesLevelAndPositiveLifetimeProfit()
        {
            ...
            Assert.That(GameSessionService.CalculateBusinessValuation(business), Is.EqualTo(450000));
        }
        ...
        public void BusinessStaffing_AddsCapacityRevenueAndPayrollWithinActionPointLimit()
        {
            ...
            save.state.skills.Add(new SkillState { skillId = "professional", experience = 50 });
            service.Start(save);
            Assert.IsTrue(service.TryPerformBusinessAction(BusinessActionType.Start, out _));
            Assert.IsTrue(service.TryPerformBusinessAction(BusinessActionType.HireStaff, out _));
            ...
            Assert.IsTrue(service.TryPerformBusinessAction(BusinessActionType.Work, out _));
            Assert.IsTrue(service.TryPerformBusinessAction(BusinessActionType.Work, out _));
            Assert.IsFalse(service.TryPerformBusinessAction(BusinessActionType.Work, out var exhausted));
        ...
        public void BusinessLocationExpansionAndSeededDisruptionAffectMonthlyResult()
        {
            ...
            service.Start(save);
            Assert.IsTrue(service.TryPerformBusinessAction(
                BusinessActionType.ExpandLocation, out var expanded), expanded);
        ...
        [Test]
        public void RentalProperty_BuyManageAndSellAreTransactionalAndIncludedInNetWorth()
        {
            var repository = new RecordingSaveRepository();
            var service = new GameSessionService(new InMemoryEventCatalog(), repository);
            var save = CreateValidSave();
            save.state.finances.cashMinorUnits = 2000000;
            service.Start(save);
            var startingNetWorth = GameSessionService.CalculateNetWorth(save.state);

            Assert.IsTrue(service.TryPerformPropertyAction(
                PropertyActionType.BuyStarterRental, null, out var bought), bought);
            var property = service.ActiveSave.state.propertyPortfolio.properties[0];
            Assert.That(service.ActiveSave.state.finances.cashMinorUnits, Is.EqualTo(500000));
            Assert.That(property.mortgageBalanceMinorUnits, Is.EqualTo(6000000));
            Assert.That(GameSessionService.CalculateNetWorth(service.ActiveSave.state),
                Is.EqualTo(startingNetWorth));

            property.occupied = false;
            Assert.IsTrue(service.TryPerformPropertyAction(
                PropertyActionType.FindTenant, property.propertyId, out var tenant), tenant);
            Assert.That(service.ActiveSave.state.propertyPortfolio.properties[0].occupied, Is.True);
            property = service.ActiveSave.state.propertyPortfolio.properties[0];
            var cashBeforeSale = service.ActiveSave.state.finances.cashMinorUnits;
            var equity = property.currentValueMinorUnits - property.mortgageBalanceMinorUnits;
            Assert.IsTrue(service.TryPerformPropertyAction(
                PropertyActionType.Sell, property.propertyId, out var sold), sold);
            Assert.That(service.ActiveSave.state.finances.cashMinorUnits, Is.EqualTo(cashBeforeSale + equity));
            Assert.That(service.ActiveSave.state.propertyPortfolio.properties[0].status, Is.EqualTo("sold"));
            Assert.That(JsonUtility.FromJson<SaveEnvelope>(repository.LastCommittedSave)
                .state.propertyPortfolio.ledger, Has.Count.EqualTo(3));
        }

        [Test]
        public void RentalProperty_MonthlyEconomicsAreDeterministicAndLedgerIsBounded()
        {
            SaveEnvelope Run()
            {
                var service = new GameSessionService(
                ...
                service.Start(save);
                Assert.IsTrue(service.TryPerformPropertyAction(
                    PropertyActionType.BuyStarterRental, null, out _));
                for (var month = 0; month < 61; month++) AdvanceAndAssert(service);
                return service.ActiveSave;
            }

            var first = Run();
            var repeated = Run();
            var property = first.state.propertyPortfolio.properties[0];
            var repeatedProperty = repeated.state.propertyPortfolio.properties[0];
            Assert.That(first.state.propertyPortfolio.ledger, Has.Count.EqualTo(60));
            Assert.That(property.currentValueMinorUnits, Is.EqualTo(repeatedProperty.currentValueMinorUnits));
            Assert.That(property.mortgageBalanceMinorUnits, Is.EqualTo(repeatedProperty.mortgageBalanceMinorUnits));
            Assert.That(property.lifetimeNetIncomeMinorUnits, Is.EqualTo(repeatedProperty.lifetimeNetIncomeMinorUnits));
            Assert.That(property.condition, Is.InRange(0, 100));
        }

        [Test]
        public void RentalProperty_PurchaseRollsBackWhenPersistenceFails()
        {
            var repository = new RecordingSaveRepository { ShouldCommit = false };
            var service = new GameSessionService(new InMemoryEventCatalog(), repository);
            var save = CreateValidSave();
            ...
            Assert.IsFalse(service.TryPerformPropertyAction(
                PropertyActionType.BuyStarterRental, null, out _));
            Assert.That(service.ActiveSave.state.finances.cashMinorUnits, Is.EqualTo(2000000));
            Assert.That(service.ActiveSave.state.propertyPortfolio.properties, Is.Empty);
        }
        ...
        public void DailyGoal_ProgressesFromRealActivityAndClaimsRewardExactlyOnceAcrossReload()
        {
            ...
            var save = CreateValidSave();
            service.Start(save);
            Assert.IsTrue(service.TryPerformActivity(ActivityType.Rest, out var activity), activity);
        ...
        public void MainAndLifeGoals_UseCareerAndNetWorthProgressWithDestinationMetadata()
        {
            ...
            service.Start(save);
            Assert.IsTrue(service.TryPerformCareerAction(CareerActionType.WorkHard, out var worked), worked);
        ...
        public void AchievementPrizes_AreDefinedForEveryAuthoredAchievement()
        {
            ...
            Assert.That(ids.All(id => GameSessionService.GetAchievementRewardMinorUnits(id) > 0), Is.True);
        }
        ...
        public void FirstLifeOrientation_CompletesOnceAndSurvivesReload()
        {
            ...
            var service = new GameSessionService(new InMemoryEventCatalog(), repository,
                utcNow: () => DateTimeOffset.Parse("2026-07-14T12:00:00Z"));
            var save = NewLifeFactory.Create(new NewLifeRequest(), "0.1.0",
        ...
        {
            var gatedEvent = RepresentativeEvents.CreateSalaryNegotiation();
            gatedEvent.requirementsJson =
                "{\"studyTrack\":\"academic\",\"minimumQualificationExperience\":50}";
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(gatedEvent);

            var lockedService = new GameSessionService(catalog, new RecordingSaveRepository());
            var lockedSave = CreateValidSave();
            lockedSave.state.calendar.monthOfYear = 12;
            lockedSave.state.education = new EducationState
            {
                studyTrack = "academic",
                qualificationExperience = 49
            };
            ...
            var wrongTrackService = new GameSessionService(catalog, new RecordingSaveRepository());
            ...
            wrongTrackSave.state.education = new EducationState
            {
            ...
            var eligibleService = new GameSessionService(catalog, new RecordingSaveRepository());
            var eligibleSave = CreateValidSave();
            eligibleSave.state.calendar.monthOfYear = 12;
            eligibleSave.state.education = new EducationState
            {
                studyTrack = "academic",
            ...
            Assert.That(eligibleEvent?.id, Is.EqualTo(RepresentativeEvents.SalaryNegotiationId));
        }

        [TestCase("general", RepresentativeEvents.AppliedFinanceChallengeId)]
        [TestCase("academic", RepresentativeEvents.CommunityHealthChallengeId)]
        [TestCase("vocational", RepresentativeEvents.SustainableTradesChallengeId)]
        public void DisciplineChallenge_RequiresMatchingTrackAndExperience(
            string studyTrack, string expectedEventId)
        {
            var catalog = new InMemoryEventCatalog();
            foreach (var evt in RepresentativeEvents.CreateLaunchAlphaCatalog())
            {
        ...
        {
            Assert.IsTrue(GameSessionService.TryGetNextCareerStep(
        ...
        public void FinanceCareerLadder_IsReachableWithinTwoYearsOfMonthlyActions()
        {
            ...
            foreach (var expectedRole in expectedRoles)
            {
                ...
                {
                    Assert.IsTrue(service.TryPerformCareerAction(
                        CareerActionType.WorkHard, out var workSummary), workSummary);
                    AdvanceAndAssert(service);
                    months++;
                }
                Assert.IsTrue(service.TryPerformCareerAction(
                    CareerActionType.AskForPromotion, out var promotionSummary), promotionSummary);
        ...
        public void CareerPromotion_RequiresProgressAndResetsItAfterPromotion()
        {
            ...
            Assert.IsFalse(service.TryPerformCareerAction(
                CareerActionType.AskForPromotion, out var lockedSummary));
        ...
        public void CareerRetirement_IsAgeGatedAndStopsSalaryProgression()
        {
            ...
            Assert.IsFalse(service.TryPerformCareerAction(
                CareerActionType.Retire, out var lockedSummary));
            Assert.That(lockedSummary, Does.Contain("age 65"));
            service.ActiveSave.state.character.age = 65;
            Assert.IsTrue(service.TryPerformCareerAction(
                CareerActionType.Retire, out var retirementSummary), retirementSummary);
        ...
        {
            Assert.That(
                GameSessionService.CalculateHourlyRateMinorUnits(annualSalary),
        ...
        {
            Assert.That(GameSessionService.GetAnnualHealthDecline(age), Is.EqualTo(expectedDecline));
        }
        ...
        public void AnnualHealthDecline_CanEndLifeAndBlocksFurtherActions()
        {
            ...
            save.state.character.lifeStage = GameSessionService.GetLifeStage(79);
            save.state.calendar.monthOfYear = 12;
            save.state.career = new CareerState();
            service.Start(save);
            ...
            Assert.IsFalse(service.TryPerformActivity(ActivityType.Rest, out var actionSummary));
        ...
        public void Achievements_UnlockPersistAndDoNotDuplicate()
        {
            ...
            Assert.IsTrue(service.TryPerformEducationAction(
                EducationActionType.Read, out var actionSummary), actionSummary);
            ...
            while (service.ActiveSave.state.character.lifeStatus == "active" && months < maximumMonths)
            {
                if (!string.IsNullOrEmpty(service.ActiveSave.state.education.awaitingDecisionId))
                {
                    var schoolChoice = service.ActiveSave.state.education.awaitingDecisionId == "education_high_transition"
                        ? SchoolPathChoice.AcademicTrack
                        : SchoolPathChoice.PublicSchool;
                ...
                var selectedChoice = nextEvent.choices.Find(choice =>
                    GameSessionService.CalculateChoicePotentialCost(
                ...
                if (selectedChoice == null)
                {
                    var availableCredit = Math.Max(0,
                        GameSessionService.CalculateHouseholdCreditLimit(service.ActiveSave.state) -
        ...
        public void SchoolPathChoice_RollsBackWhenAutosaveFails()
        {
            ...
            Assert.IsFalse(service.TryChooseSchoolPath(
                SchoolPathChoice.VocationalTrack,
                out _));
        ...
        public void ContextActivity_OvertimeRequiresEmploymentAndCommitsPayProgressAndHealth()
        {
            ...
            Assert.IsFalse(service.TryPerformActivity(ActivityType.Overtime, out var lockedSummary));
            ...
            Assert.IsTrue(service.TryPerformActivity(ActivityType.Overtime, out var summary), summary);

            Assert.That(service.ActiveSave.state.finances.cashMinorUnits,
                Is.EqualTo(cashBefore + GameSessionService.CalculateHourlyRateMinorUnits(4000000) * 4));
        ...
        public void FitnessLevel_ReducesOvertimeHealthStrain()
        {
            ...
            save.state.career.roleTitle = "Junior Associate";
            save.state.career.annualSalaryMinorUnits = 4000000;
            save.state.skills.Add(new SkillState { skillId = "fitness", experience = 50 });
            service.Start(save);
            var healthBefore = service.ActiveSave.state.character.health;
        ...
        public void ProfessionalLevel_IncreasesCareerProgressFromWorkingHard()
        {
            ...
            Assert.IsTrue(service.TryPerformCareerAction(
                CareerActionType.WorkHard, out var summary), summary);
        ...
        public void PeerRelationship_CanBecomeARivalAndReconcileLater()
        {
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "school_peer_primary", RelationshipInteractionType.Argue, out var argueSummary), argueSummary);
            ...
            Assert.IsTrue(service.TryAdvanceMonth(out _, out var advanceSummary), advanceSummary);
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "school_peer_primary", RelationshipInteractionType.Reconcile, out var reconcileSummary), reconcileSummary);
            Assert.That(service.ActiveSave.state.relationships.Find(relationship =>
        ...
        {
            var followUp = RepresentativeEvents.CreateSalaryNegotiation();
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(followUp);
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            var save = CreateValidSave();
            save.state.calendar.monthOfYear = 12;
            save.state.scheduledEvents.Add(new ScheduledEventRecord
            {
                eventId = followUp.id,
        ...
        {
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(RepresentativeEvents.CreatePeerTrustConflict());
            var save = CreateValidSave();
            ...
            Assert.IsTrue(service.TryPerformRelationshipInteraction(
                "school_peer_primary", RelationshipInteractionType.Argue, out var interactionSummary),
                interactionSummary);
            ...
            var reloaded = JsonUtility.FromJson<SaveEnvelope>(JsonUtility.ToJson(service.ActiveSave));
            var resumed = new GameSessionService(catalog, new RecordingSaveRepository());
            resumed.Start(reloaded);
            Assert.IsTrue(resumed.TryAdvanceMonth(out var nextEvent, out var advanceSummary), advanceSummary);
            Assert.That(nextEvent?.id, Is.EqualTo(RepresentativeEvents.PeerTrustConflictId));
            Assert.That(resumed.ActiveSave.state.scheduledEvents, Is.Empty);
            Assert.That(resumed.ActiveSave.state.statuses.Exists(status =>
                status.statusId == $"scheduled_event_cooldown_{RepresentativeEvents.PeerTrustConflictId}"), Is.True);
        }
        ...
        public void ScheduledMonthlyTriggers_SelectHighestPriorityFirst()
        {
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(RepresentativeEvents.CreatePeerTrustConflict());
            catalog.Upsert(RepresentativeEvents.CreatePeerJealousy());
            var save = CreateValidSave();
            save.state.character.age = 14;
            save.state.relationships.Add(new RelationshipState
            {
            ...
            save.state.scheduledEvents.Add(new ScheduledEventRecord
            {
                eventId = RepresentativeEvents.PeerJealousyId,
                earliestTriggerMonth = triggerMonth,
                latestTriggerMonth = triggerMonth + 2,
            ...
            Assert.IsTrue(service.TryAdvanceMonth(out var nextEvent, out var summary), summary);
            Assert.That(nextEvent?.id, Is.EqualTo(RepresentativeEvents.PeerJealousyId));
            Assert.That(service.ActiveSave.state.scheduledEvents.Single().eventId,
                Is.EqualTo(RepresentativeEvents.PeerTrustConflictId));
        }
        ...
        public void PeerDrama_BetrayalSchedulesAndResolvesLaterConsequence()
        {
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(RepresentativeEvents.CreatePeerTrustConflict());
            catalog.Upsert(RepresentativeEvents.CreatePeerTrustAftermath());
            catalog.Upsert(RepresentativeEvents.CreatePeerJealousy());
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            var save = CreateValidSave();
            save.state.character.age = 14;
            ...
            Assert.IsTrue(service.TryResolveChoice(
                RepresentativeEvents.PeerTrustConflictId,
                "share_secret",
            ...
            Assert.That(service.ActiveSave.state.scheduledEvents[0].eventId,
                Is.EqualTo(RepresentativeEvents.PeerTrustAftermathId));

            service.ActiveSave.state.calendar.monthOfYear = 12;
            Assert.IsTrue(service.TryAdvanceMonth(out var followUp, out var advanceSummary), advanceSummary);
            Assert.That(followUp?.id, Is.EqualTo(RepresentativeEvents.PeerTrustAftermathId));
            Assert.IsTrue(service.TryResolveChoice(
                followUp.id,
                "talk_honestly",
            ...
            Assert.That(service.ActiveSave.state.eventHistory.Exists(entry =>
                entry.eventId == RepresentativeEvents.PeerTrustAftermathId), Is.True);
            Assert.That(service.ActiveSave.state.lifeFeed.Exists(entry =>
        ...
        {
            var jealousy = RepresentativeEvents.CreatePeerJealousy();
            var catalog = new InMemoryEventCatalog();
            ...
            service.ActiveSave.state.relationships.Add(new RelationshipState
            {
                relationshipId = "school_peer_middle", displayName = "Alex",
                relationshipType = "friend", value = 50
            });
            Assert.IsTrue(service.TryAdvanceMonth(out var eventWithBothPeers, out var eventSummary), eventSummary);
            Assert.That(eventWithBothPeers?.id, Is.EqualTo(RepresentativeEvents.PeerJealousyId));
            Assert.IsTrue(service.TryResolveChoice(
        ...
        {
            var gain = RepresentativeEvents.CreateRandomGain();
            var loss = RepresentativeEvents.CreateRandomLoss();

            Assert.That(GameSessionService.GetLuckEventWeight(gain, 90),
                Is.GreaterThan(GameSessionService.GetLuckEventWeight(gain, 10)));
            Assert.That(GameSessionService.GetLuckEventWeight(loss, 90),
                Is.LessThan(GameSessionService.GetLuckEventWeight(loss, 10)));
        }
        ...
        public void PerformActivity_RejectsSecondFocusUntilMonthAdvances()
        {
            ...
            Assert.IsTrue(service.TryPerformActivity(ActivityType.Study, out var firstSummary), firstSummary);
            var secondPerformed = service.TryPerformActivity(ActivityType.Workout, out var secondSummary);
            ...
            Assert.IsTrue(service.TryAdvanceMonth(out _, out var advanceSummary), advanceSummary);
            Assert.IsTrue(service.TryPerformActivity(ActivityType.Workout, out var thirdSummary), thirdSummary);
            Assert.That(service.ActiveSave.state.character.health, Is.EqualTo(82));
        }
        ...
        {
            var evt = RepresentativeEvents.CreateHealthBurnout();
        ...
        public void StagedCatalog_HasDeterministicBoundedDistributionAcrossLifeStages(int age)
        {
            ...
            var authored = StagedEventCatalog.CreateAllStagedEvents();
            var eligible = authored.Where(evt => EventAgeEligibility.IsEligible(evt, age)).ToList();
        ...
        {
            var evt = StagedEventCatalog.CreateCareerBatch()[0];
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(evt);
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            service.Start(CreateValidSave());
        ...
        public void PlayFlow_ResolvesEventReloadsSaveAndContinuesNextMonth()
        {
            ...
            var resumedSession = new GameSessionService(catalog, repository);
        ...
        public void SavingsTransfers_AreAtomicAndWriteAuditableHistory()
        {
            ...
            Assert.IsTrue(service.TryTransferSavings(
                SavingsTransferType.Deposit, 25000, out var depositSummary), depositSummary);
            ...
            Assert.IsTrue(service.TryTransferSavingsPercentage(
                SavingsTransferType.Withdrawal, 50, out var withdrawalSummary), withdrawalSummary);
        ...
        public void SavingsTransfer_PersistenceFailureRollsBackBalancesAndHistory()
        {
            ...
            Assert.IsFalse(service.TryTransferSavings(
                SavingsTransferType.Deposit, 10000, out _));
            Assert.That(service.ActiveSave.state.finances.cashMinorUnits, Is.EqualTo(100000));
        ...
        public void SavingsHistory_RetainsNewestOneHundredEntries()
        {
            ...
            for (var index = 0; index < 101; index++)
                Assert.IsTrue(service.TryTransferSavings(
                    SavingsTransferType.Deposit, 100, out var summary), summary);
        ...
        public void AdvanceMonth_AccruesGroundedSavingsInterestAndPersistsCashFlowDetail()
        {
            ...
            Assert.That(GameSessionService.CalculateProjectedAnnualSavingsInterest(finances),
                Is.EqualTo(4212));
        }
        ...
        {
            var state = CreateValidSave().state;
            state.character.age = GameSessionService.IndexInvestmentMinimumAge;
            state.character.smarts = GameSessionService.IndexInvestmentMinimumSmarts;
            ...
            state.finances.savingsMinorUnits =
                GameSessionService.IndexInvestmentMinimumEmergencySavingsMinorUnits;
            state.finances.cashMinorUnits = 1000;

            Assert.IsTrue(GameSessionService.TryGetIndexInvestmentRequirement(
                state, out var requirement), requirement);

            state.character.age--;
            Assert.IsFalse(GameSessionService.TryGetIndexInvestmentRequirement(state, out _));
            state.character.age++;
        ...
        {
            var first = GameSessionService.CalculateAnnualIndexReturnBasisPoints(42, 30);
            var second = GameSessionService.CalculateAnnualIndexReturnBasisPoints(42, 30);
            ...
            long startingIndex)
        {
            ...
            for (var month = 0; month < 240; month++)
            {
                ...
                if ((month + 1) % 12 == 0 && index > 0)
                {
                    ...
                    index *= 1m + GameSessionService.CalculateAnnualIndexReturnBasisPoints(seed, age) / 10000m;
                }
            }
        ...
        private static Event SelectStagedEvent(int age, int seed)
        {
            var catalog = new InMemoryEventCatalog();
            foreach (var evt in StagedEventCatalog.CreateAllStagedEvents()) catalog.Upsert(evt);
            var service = new GameSessionService(catalog, new RecordingSaveRepository());
            var save = CreateValidSave();
        ...
        private static bool TryAnyMatchSwap(GameSessionService service, out string summary)
        {
        ...
        private static void AssertReloadedMatchState(SaveEnvelope save, string expectedState)
        {
            var reloadedSave = JsonUtility.FromJson<SaveEnvelope>(JsonUtility.ToJson(save));
            var reloaded = new GameSessionService(
                new InMemoryEventCatalog(), new RecordingSaveRepository());
        ...
        private static SaveEnvelope CreateValidSave()
        {
            return new SaveEnvelope
            {
                ...
                state = new GameState
                {
                    ...
                    finances = new FinancesState { cashMinorUnits = 100000 },
                    calendar = new CalendarState { monthOfYear = 1 },
        ...
        private static void AdvanceAndAssert(GameSessionService service)
        {
            Assert.IsTrue(service.TryAdvanceMonth(out var nextEvent, out var summary), summary);
            Assert.That(nextEvent, Is.Null);
        }
        ...
        private static long SimulateBusiness(int seed, int months)
        {
            ...
            {
                var profit = GameSessionService.CalculateBusinessMonthlyProfit(
        ...
        {
            save.state.family.children.Add(new ChildState
            {
                childId = childId,
                displayName = "Ari",
                path = "adoption",
        ...
        private static InMemoryEventCatalog CreateRepresentativeCatalog()
        {
            var catalog = new InMemoryEventCatalog();
            catalog.Upsert(RepresentativeEvents.CreateSalaryNegotiation());
            catalog.Upsert(RepresentativeEvents.CreateHealthBurnout());
            catalog.Upsert(RepresentativeEvents.CreateMoneyFastReturn());
            catalog.Upsert(RepresentativeEvents.CreateSchoolGroupProject());
            catalog.Upsert(RepresentativeEvents.CreateChildhoodGrownFolksTable());
            catalog.Upsert(RepresentativeEvents.CreateRandomGain());
            catalog.Upsert(RepresentativeEvents.CreateRandomLoss());
            catalog.Upsert(RepresentativeEvents.CreateRandomGainRefund());
            catalog.Upsert(RepresentativeEvents.CreateRandomLossRepair());
            catalog.Upsert(RepresentativeEvents.CreateLuckCrossroads());
            catalog.Upsert(RepresentativeEvents.CreateChildhoodDiscovery());
            catalog.Upsert(RepresentativeEvents.CreateChildhoodComfort());
            catalog.Upsert(RepresentativeEvents.CreatePeerTrustConflict());
            catalog.Upsert(RepresentativeEvents.CreatePeerTrustAftermath());
            catalog.Upsert(RepresentativeEvents.CreateComingOfAgeGender());
            catalog.Upsert(RepresentativeEvents.CreateComingOfAgeOrientation());
            ...
            catalog.Upsert(RepresentativeEvents.CreateWedding());
            catalog.Upsert(RepresentativeEvents.CreateMarriageCrossroads());
            catalog.Upsert(RepresentativeEvents.CreateHomeDeferredMaintenance());
            return catalog;
        }
        ...
        private sealed class RecordingSaveRepository : ISaveRepository
        {
            ...
            {
                var save = JsonUtility.FromJson<SaveEnvelope>(serializedSave);
                var result = SaveValidator.ValidateSave(save);
                validationSummary = SaveValidator.GetValidationSummary(result, save?.saveId);
