...
{
    /// <summary>
    /// Unit tests for EventValidator and Event schema compliance.
    /// </summary>
    public class EventValidatorTests
    {
        ...
        {
            var result = AlphaContentCoverage.Validate(
                RepresentativeEvents.CreateLaunchAlphaCatalog());
        ...
        {
            var events = new List<Event>(RepresentativeEvents.CreateLaunchAlphaCatalog());
            var repeatable = events.Find(item => item.id == RepresentativeEvents.RandomGainId);
            repeatable.cooldownYears = 0;
            var chainStart = events.Find(item => item.id == RepresentativeEvents.PeerTrustConflictId);
            chainStart.choices[0].outcomes[0].followUps[0].eventId = "missing_follow_up";

            var result = AlphaContentCoverage.Validate(events);
        ...
        public void ProductionValidation_RejectsRiskMismatchMalformedEligibilityAndUnsafeLocalizationId()
        {
            ...
            var result = EventValidator.ValidateProductionEvent(evt);
        ...
        {
            var result = EventValidator.ValidateEvent(null);
        ...
        {
            var result = EventValidator.ValidateEvent(RepresentativeEvents.CreateHealthBurnout());

            Assert.IsTrue(result.isValid, EventValidator.GetValidationSummary(result, RepresentativeEvents.HealthBurnoutId));
        }

        [Test]
        public void ValidateEvent_PassesRepresentativeMoneyEvent()
        {
            var result = EventValidator.ValidateEvent(RepresentativeEvents.CreateMoneyFastReturn());

            Assert.IsTrue(result.isValid, EventValidator.GetValidationSummary(result, RepresentativeEvents.MoneyFastReturnId));
        }

        [Test]
        public void ValidateEvent_PassesRepresentativeSchoolEvent()
        {
            var result = EventValidator.ValidateEvent(RepresentativeEvents.CreateSchoolGroupProject());

            Assert.IsTrue(result.isValid, EventValidator.GetValidationSummary(result, RepresentativeEvents.SchoolGroupProjectId));
        }

        [Test]
        public void ValidateEvent_PassesRepresentativeChildhoodEvent()
        {
            var result = EventValidator.ValidateEvent(RepresentativeEvents.CreateChildhoodGrownFolksTable());

            Assert.IsTrue(result.isValid, EventValidator.GetValidationSummary(result, RepresentativeEvents.ChildhoodGrownFolksTableId));
        }
        ...
        {
            var evt = gain
                ? RepresentativeEvents.CreateRandomGain()
                : RepresentativeEvents.CreateRandomLoss();
            var result = EventValidator.ValidateEvent(evt);

            Assert.IsTrue(result.isValid, EventValidator.GetValidationSummary(result, evt.id));
        }
        ...
        public void ValidateEvent_PassesExpandedRandomAndLuckEvents()
        {
            var events = new[]
            {
                ...
                RepresentativeEvents.CreateLuckCrossroads(),
                RepresentativeEvents.CreateChildhoodDiscovery(),
                RepresentativeEvents.CreateChildhoodComfort()
            };
        ...
        public void ValidateEvent_PassesAuthoredRelationshipSafetySet()
        {
            var events = new[]
            {
                RepresentativeEvents.CreatePromInvitation(),
                RepresentativeEvents.CreateFirstKiss(),
                RepresentativeEvents.CreateProposal(),
                RepresentativeEvents.CreateWedding(),
                RepresentativeEvents.CreateMarriageCrossroads()
            };
        ...
        public void ValidateEvent_PassesAuthoredEducationDisciplineSet()
        {
            var events = new[]
            {
                RepresentativeEvents.CreateAppliedFinanceChallenge(),
                RepresentativeEvents.CreateCommunityHealthChallenge(),
                RepresentativeEvents.CreateSustainableTradesChallenge()
            };
        ...
        public void GetValidationSummary_CreatesHumanReadableSummary()
        {
            ...
            var summary = EventValidator.GetValidationSummary(result, "test_event");
