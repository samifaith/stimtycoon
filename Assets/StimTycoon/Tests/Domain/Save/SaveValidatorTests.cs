...
namespace StimTycoon.Tests.Domain.Save
{
    ...
    public class SaveValidatorTests
    {
        ...
        {
            var result = SaveValidator.ValidateSave(null);
        ...
        public void ValidateSave_RejectsDuplicateOrOverCapacityHomeInventory()
        {
            ...
            duplicate.state.home.inventory.Add(new HomeInventoryItemState
            {
                itemId = "starter_books", category = "book", quantity = 1,
                capacity = 1, acquisitionSource = "test"
            });
            var duplicateResult = SaveValidator.ValidateSave(duplicate);
            ...
            var capacityResult = SaveValidator.ValidateSave(overCapacity);
        ...
        {
            var save = CreateValidSave();
            for (var index = 0; index <= ScheduledEventRecord.MaxScheduledEvents; index++)
        ...
        {
            var save = CreateValidSave();
            save.state.skills.Add(new SkillState { skillId = "negotiation", experience = -1 });
        ...
        [Test]
        public void ValidateSave_RejectsInvalidPropertyAndOversizedBusinessPortfolio()
        {
            var save = CreateValidSave();
            for (var index = 0; index <= BusinessPortfolioState.MaxBusinesses; index++)
                save.state.businessPortfolio.businesses.Add(new BusinessState
                {
                    businessId = $"business_{index}", businessType = "local_services",
                    displayName = "Business", status = "operating"
                });
            save.state.propertyPortfolio.properties.Add(new RentalPropertyState
            {
                propertyId = "property_1", propertyType = "rental_home", displayName = "Rental",
                status = "owned", purchasePriceMinorUnits = 100000,
                currentValueMinorUnits = 100000, mortgageBalanceMinorUnits = 120000,
                condition = 80
            });
            ...
            Assert.IsFalse(result.isValid);
            Assert.That(result.errors, Has.Some.Contains("state.businessPortfolio must contain at most 3 businesses"));
            Assert.That(result.errors, Has.Some.Contains("state.propertyPortfolio contains an invalid property"));
        }
        ...
        {
            var save = CreateValidSave();
            save.state.goals.Add(new GoalState
            {
        ...
        {
            var save = CreateValidSave();
            save.state.relationships.Add(new RelationshipState { relationshipId = "parent", value = 50 });
            save.state.relationships.Add(new RelationshipState { relationshipId = "parent", value = 60 });
        ...
        public void ValidateSave_AcceptsCompleteAdultCompatibleDiscoveryIdentity()
        {
            ...
            Assert.IsTrue(result.isValid, SaveValidator.GetValidationSummary(result, save.saveId));
        }
        ...
        public void ValidateSave_AcceptsDependentAndIndependentChildBoundaries()
        {
            ...
            Assert.IsTrue(SaveValidator.ValidateSave(dependent).isValid);
            Assert.IsTrue(SaveValidator.ValidateSave(adult).isValid);
        }
        ...
        {
            var save = CreateValidSave();
            save.state.lifeDecisions.Add(new LifeDecisionState
            {
        ...
        public void ValidateSave_AcceptsActionProgressAndRejectsDuplicateInstances()
        {
            ...
            var valid = SaveValidator.ValidateSave(save);
            Assert.IsTrue(valid.isValid, SaveValidator.GetValidationSummary(valid, save.saveId));
            ...
            var duplicate = SaveValidator.ValidateSave(save);
        ...
        {
            var save = CreateValidSave();
            save.state.statuses.Add(new StatusState { statusId = "worried", remainingMonths = 0 });
        ...
        public void GetValidationSummary_CreatesHumanReadableSummary()
        {
            ...
            var summary = SaveValidator.GetValidationSummary(result, "save_001");
        ...
        private static SaveEnvelope CreateValidSave()
        {
            ...
            {
                saveFormatVersion = SaveSchema.SupportedSaveFormatVersion,
                minimumReaderVersion = SaveSchema.SupportedMinimumReaderVersion,
        ...
        private static void AddChild(SaveEnvelope save, int age, string custodyStatus)
        {
