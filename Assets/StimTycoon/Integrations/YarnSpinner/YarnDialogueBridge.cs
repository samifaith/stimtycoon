...
namespace StimTycoon.YarnSpinner
{
    ...
    /// </summary>
    public sealed class YarnDialogueBridge : MonoBehaviour, IDialogueBridge
    {
        [SerializeField] private DialogueRunner dialogueRunner;

        private EventRuntimeService eventRuntimeService;
        private GameSessionService gameSessionService;

        public event Action<StimTycoon.Events.ChoiceResolution> ChoiceResolved;

        public void Initialize(EventRuntimeService runtimeService)
        {
            eventRuntimeService = runtimeService ?? throw new ArgumentNullException(nameof(runtimeService));
        }

        public void Initialize(
            EventRuntimeService runtimeService,
