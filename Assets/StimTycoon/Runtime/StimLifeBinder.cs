...
{
    internal sealed class StimLifeBinder
    {
        ...
        public StimLifeBinder(VisualElement root, VisualTreeAsset feedRowTemplate)
        {
        ...
        public void RenderFeed(IReadOnlyList<StimLifeFeedEntry> sourceEntries)
        {
            if (lifeFeedList == null) return;
            lifeFeedList.Clear();
            var entries = StimLifeFeedPresentation.GetNewestFirst(sourceEntries);
            if (entries.Count == 0)
            {
