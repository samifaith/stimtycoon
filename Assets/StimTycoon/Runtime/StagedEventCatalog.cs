...
namespace StimTycoon.Runtime
{
    ...
    /// </summary>
    public static class StagedEventCatalog
    {
        ...
        public static IReadOnlyList<Event> CreateChildhoodBatch()
        {
            var events = new List<Event>(ChildhoodSpecs.Length);
            for (var index = 0; index < ChildhoodSpecs.Length; index++)
                events.Add(CreateChildhoodEvent(ChildhoodSpecs[index], index));
            return events;
        }

        public static IReadOnlyList<Event> CreateSchoolBatch()
        {
            var events = new List<Event>(SchoolSpecs.Length);
            for (var index = 0; index < SchoolSpecs.Length; index++)
                events.Add(CreateSchoolEvent(SchoolSpecs[index], index));
            return events;
        }

        public static IReadOnlyList<Event> CreateCareerBatch()
        {
            var events = new List<Event>(CareerSpecs.Length);
            for (var index = 0; index < CareerSpecs.Length; index++)
                events.Add(CreateCareerEvent(CareerSpecs[index], index));
            return events;
        }

        public static IReadOnlyList<Event> CreateHealthBatch()
        {
            var events = new List<Event>(HealthSpecs.Length);
            for (var index = 0; index < HealthSpecs.Length; index++)
                events.Add(CreateHealthEvent(HealthSpecs[index]));
            return events;
        }

        public static IReadOnlyList<Event> CreateMoneyBatch()
        {
            var events = new List<Event>(MoneySpecs.Length);
            for (var index = 0; index < MoneySpecs.Length; index++)
                events.Add(CreateMoneyEvent(MoneySpecs[index], index));
            return events;
        }

        public static IReadOnlyList<Event> CreateAllStagedEvents()
        {
        ...
        private static Event CreateChildhoodEvent(EventSpec spec, int index)
        {
            return new Event
            {
                id = spec.id,
        ...
        private static Event CreateSchoolEvent(EventSpec spec, int index)
        {
            return new Event
            {
                id = spec.id,
        ...
        private static Event CreateCareerEvent(EventSpec spec, int index)
        {
            return new Event
            {
                id = spec.id,
        ...
        private static Event CreateHealthEvent(EventSpec spec)
        {
            return new Event
            {
                id = spec.id,
        ...
        private static Event CreateMoneyEvent(EventSpec spec, int index)
        {
            return new Event
            {
                id = spec.id,
        ...
        private static string GetValueRule(EffectType effectType, float value)
        {
            switch (effectType)
            {
                case EffectType.StatDelta:
                    return value < 0f ? EffectValueRules.StagedStatLoss :
                        EffectValueRules.StagedStatGain;
                case EffectType.SkillXp:
                    return EffectValueRules.StagedSkillXpGain;
                case EffectType.CareerProgressDelta:
                    return EffectValueRules.StagedCareerProgressGain;
                case EffectType.CashDelta:
                    if (value < 0f) return EffectValueRules.StagedCashSmallCost;
                    return value >= 1000f ? EffectValueRules.StagedCashMediumGain :
                        EffectValueRules.StagedCashSmallGain;
                default:
