...
{
    internal sealed class GoalsBinder
    {
        ...
        public GoalsBinder(VisualElement root, VisualTreeAsset rowTemplate)
        {
        ...
        public void Render(
            ...
            string board,
            Action<GoalState> handleGoal,
            Action<GoalState> togglePin,
            Action<string> claimAchievement)
        {
            ...
            if (board == "achievements" && achievements != null)
            {
                for (var index = achievements.Count - 1; index >= 0; index--)
                {
                    ...
                    var reward = GameSessionService.GetAchievementRewardMinorUnits(achievement.achievementId);
                    var row = UiComponentFactory.CreateAchievementRow(
                        achievement.achievementId,
                        "🏆",
                        GameSessionService.GetAchievementDisplayName(achievement.achievementId),
                        "Achievement",
        ...
        private static void ApplyTab(Button tab, bool selected) =>
            PresentationStateStyler.Apply(tab, selected ? PresentationState.Selected : PresentationState.Available);
    }
}
