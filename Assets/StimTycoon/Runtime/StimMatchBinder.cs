...
{
    internal sealed class StimMatchBinder
    {
        ...
        public StimMatchBinder(VisualElement root)
        {
        ...
        public void Render(StimMatchSessionState session, Action<int, int> swap)
        {
            session ??= new StimMatchSessionState();
            ...
            for (var index = 0; index < session.board.Count; index++)
            {
                ...
                StimPresentationStateStyler.Apply(tile, index == selectedIndex
